...
########################################################################################################################
    ...
    if m.text == (get_db_botname() or "سفير"):
        texting = ["اؤمر " + (get_db_botname() or "سفير") + " شتريد؟❤️🥺",
                   "اى يقلب " + (get_db_botname() or "سفير") + "❤️",
                   ...
                   "قلبى بينادى على " + (get_db_botname() or "سفير") + "😘",
                   "نعسان محدش يصحينى🙄"
                   ]
        await m.reply_text(random.choice(texting))
        return

    if m.text == "بوت غادر" or m.text == (get_db_botname() or "سفير") + " غادر":
        if secsudo(m):
            try:
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
    ...
    if m.text == "سفير":
        s = await c.get_users("7423117691")
        ss = (await c.get_chat("7423117691")).bio
        sender_name = f"[{m.from_user.first_name}](tg://user?id={m.from_user.id})"
        name_chat = m.chat.title
        id_chat = m.chat.id
        if m.chat.type == enums.ChatType.SUPERGROUP:
        ...
        if m.chat.username:
            ...
            try:
                ...
                link_group = "لايوجد"
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(f"{s.first_name}", user_id=7423117691)],
        ])
        try:
            await c.get_chat_photos("7423117691", limit=1).__anext__()
        except:
            await m.reply_text(f"{ss}", reply_markup=keyboard),
            await c.send_message("7423117691", f"❤️╖ نداء لك ايها المبرمج\n📟╢ بواسطة {sender_name}\n📆╢ يوم *{today}*\n🕑╢ الساعه *{clock}*\n💌╢ اسم الجروب {name_chat}\n🔰╢ ايدي الجروب *{id_chat}*\n⚙️╢ عدد اعضاء الجروب *{num_member}*\n⛓╢ رابط المسج {message_link}\n🔍╜ الرابط {link_group}", parse_mode=enums.ParseMode.MARKDOWN)
        else:
            async for photo in c.get_chat_photos("7423117691", limit=1):
